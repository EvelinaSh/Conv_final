...
class algController {
    ...
    async save(req, res, next) {
            try{
                const {query_name, desc_query, table_var, goal_list, query_body, nom_query, nom_qr, first_name, userId, complexity} = req.body
                const alg = await Alg.create({query_name, desc_query, table_var, goal_list, query_body, nom_query, nom_qr, first_name, userId, complexity})
                return res.json(alg)
            }
            catch (e) {
    ...
    async update(req, res) {
        try{
            const {query_name, desc_query, table_var, goal_list, query_body, nom_query, nom_qr, first_name, complexity} = req.body
            const alg = await Alg.update({desc_query, table_var, goal_list, query_body, nom_query, nom_qr, first_name, complexity},
                {where: {query_name}}
            )
            return res.json(alg)
        }
    ...
    async generate(req, res) {
        try{
            ...
            convert_to_sql(query_object).then(alg => {
                res.json(alg)
                })
        }
        catch (e) {
    ...
    async view(req, res, next){
        try{
            const {query_name, query_object} = req.body
            const sql = "create view ':qn' as ':qs'"
            let query_sql
            convert_to_sql(query_object).then(async query => {
                query_sql.json(query)
                const alg = await db.query(sql, {replacements: {qn: [query_name], qs: [query_sql]}}).then()
                return res.json(alg)
            })
        }
        catch (e) {
...
function make_beautifull_sql(sql) {
    ...
    console.log('before beautifull', sql);
...
RelationalAlgebraQuery.prototype.except_intersect_transformation = function(type) {
    return new Promise(function(resolve, reject) {
        ...
        const left_operand = new RelationalAlgebraQuery({
            title: '',
            alias: this.alias,
...
RelationalAlgebraQuery.prototype.division_transformation = function() {
    return new Promise(function(resolve, reject) {
        ...
        let target_list_arr = this.target_list.split(",");
...
const make_copy = function(alias, sql) {
...
RelationalAlgebraQuery.prototype.make_sql = function() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        this.proection_transformation()
            ...
            var result = re.exec(this.query_body)
            let selection = result[3] ? result[3] : "";
...
RelationalAlgebraQuery.prototype.proection_transformation = function() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (re.test(this.query_body))
            proection_tl = re.exec(this.query_body)[1];
...
function convert_to_sql(query_object) {
    //console.log('here');
    const query = new RelationalAlgebraQuery(query_object);
